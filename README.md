...
The first approved build is preserved at Git tag `prototype-01-zone-transitions` (commit `23ed7a1`). The current experiment is mechanics lab 007: four selectable enemy encounters. Choose Dart squadron, Interceptors, Mine layer, or Convoy hauler on the start screen. Each test isolates that encounter; retry it or use **Encounters** to choose another. **Highway run** keeps the existing traffic/ramp experiment available. See `docs/ENCOUNTERS.md` for behaviors and tuning values.
