...
The campaign contains **24 authored levels across six areas**: **Dockyards** (steering and phases), **Conduits** (inner-tube passages), **Broken Span** (hurdles, gaps and landings), **Relay Grid** (phase rhythm and steering), **Outer Ring** (exterior routes and speed control), and **Nexus** (combined skills and surface transitions). Clear an area's four levels to unlock the next. Checkpoints refill boost and continue within an area; incompatible starting surfaces reset the ship pose. Area boundaries stop at a completion screen. Retries restart the current level. The previous campaign and repeating Overload rounds have been removed.

Use **Practice** to select any of the 24 courses without recording completion or best times. A direct example is `/racer.html?practice=1&level=broken-span-hurdles`. Completion and records use stable course IDs; existing Dockyards/Conduits clears and revised-layout records remain intact. Each area has representative artwork and four level buttons, with a total cleared/24 counter. Contextual prompts introduce new actions during the run. See `docs/CAMPAIGN_AREAS.md` for the level list and tuning notes.
