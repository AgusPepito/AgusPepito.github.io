...
The first approved build is preserved at Git tag `prototype-01-zone-transitions` (commit `23ed7a1`). The current experiment is mechanics lab 008: harder selectable encounters, with overlapping Dart waves, coordinated Interceptors, two Mine layers, and guarded Convoy locks. Choose a test on the start screen; retry it or use **Encounters** to choose another. Results distinguish clears, escapes and partial raids. **Highway run** keeps the existing traffic/ramp experiment available. See `docs/ENCOUNTERS.md` for behaviors and tuning values.
