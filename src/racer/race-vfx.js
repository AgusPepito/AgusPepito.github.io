...
export class RaceVfx {
  constructor(scene, ship, exhausts, quality) {
    ...
    this.trails=exhausts.map(()=>{
      const count=28,geometry=new THREE.BufferGeometry(),positions=new Float32Array(count*6),colors=new Float32Array(count*6),indices=[];
      ...
      const mesh=new THREE.Mesh(geometry,new THREE.MeshBasicMaterial({vertexColors:true,transparent:true,
        opacity:.45,depthWrite:false,side:THREE.DoubleSide,blending:THREE.AdditiveBlending}));
      mesh.frustumCulled=false;mesh.visible=false;this.group.add(mesh);
      return {mesh,positions,colors,points:Array.from({length:count},()=>new THREE.Vector3()),times:new Float32Array(count),head:0,used:0};
    });
  ...
  update(race, f, camera, reduced) {
    ...
    if(!reset&&!reduced&&!race.demo&&race.state==='running'){
      ...
      if(previous.airborne&&!race.airborne&&race.landing>0){
        this.scratch.copy(f.p).addScaledVector(f.normal,.07);
        this.pulse(this.scratch,f.normal,this.color,4.2,.35);
      }
    ...
    for(let engine=0;engine<this.trails.length;engine++){
      ...
      for(let i=0;i<count;i++){
        ...
        const age=Math.max(0,race.time-trail.times[index]),fade=Math.max(0,1-age/.16);
        const width=(.09+race.thrustBlend*.075)*fade;
        for(let edge=0;edge<2;edge++){
          ...
          const value=fade*fade*strength;
          trail.colors.set([this.color.r*value,this.color.g*value,this.color.b*value],i*6+edge*3);
        }
      }
    ...
    this.last={time:race.time,s:race.s,u:race.u,phase:race.phase,passed:race.passed,airborne:race.airborne,
      boost:race.boostActive,state:race.state,demo:Boolean(race.demo)};
  }
}
