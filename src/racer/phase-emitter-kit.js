...
export function emitterAssembly(THREE,{shape='flat',width=wallHalfWidth(shape)*2,phase=0,color=PHASES[phase].hex,projection=true,road=false,merge=false}={}){
  const source=emitter(THREE,{width,color,projection,curl:shape==='inside'?1:shape==='outside'?-1:0,
    closed:shape!=='flat'&&Math.abs(width-wallHalfWidth(shape)*2)<.01});
  if(shape!=='flat'){
    source.updateMatrixWorld(true);
    source.traverse(m=>{
      if(!m.isMesh)return;
      m.geometry.applyMatrix4(m.matrixWorld);m.position.set(0,0,0);m.rotation.set(0,0,0);m.scale.set(1,1,1);
      // The curtain already has a fine circumference grid and source-space UVs.
