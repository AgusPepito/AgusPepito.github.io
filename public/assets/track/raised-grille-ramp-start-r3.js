...
export default function generate(THREE) {
  const family='grille', kind='ramp-start'; const g = new THREE.Group(); g.name = 'raised-'+family+'-'+kind+'-r3';
  ...
  if(kind==='ramp-start'||kind==='ramp-end') {
    ...
    const rampDirection = kind==='ramp-start' ? 1 : -1;
    const rampTip = -6 * rampDirection, rampLength = 8;
    // The full-height shoulder is eight metres inward from the low tip.
    // Use this same interval for both the face split and the deformation.
    const shoulder = rampTip + rampDirection * rampLength;
    ...
    g.traverse(n=>{
      ...
      for(let i=0;i<p.count;i++) {
        ...
        const t=THREE.MathUtils.clamp((x-rampTip)*rampDirection/rampLength,0,1);
