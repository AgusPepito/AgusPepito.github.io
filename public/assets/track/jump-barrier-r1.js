...
function builder(THREE,prefix){
  ...
  function plateGeometry(w,h,holes=[]){
    ...
    const g=new THREE.ExtrudeGeometry(s,{depth:.09,bevelEnabled:true,bevelSize:.018,bevelThickness:.018,bevelSegments:1,steps:1});g.translate(0,0,-.108);return g;
  }
  ...
  function pipe(x,low,high,z){
    mesh('protected-cooling-conduit',new THREE.CylinderGeometry(.105,.105,high-low,12),'steel',x,(low+high)/2,z);
    for(const y of [low+.15,high-.15]){
      mesh('conduit-coupling',new THREE.CylinderGeometry(.15,.15,.18,12),'brass',x,y,z);
      for(const dy of [-.12,.12])mesh('conduit-seal',new THREE.CylinderGeometry(.13,.13,.04,12),'dark',x,y+dy,z);
    }
  }
...
export default function generate(THREE,{width=18,height=2.4,startCap=true,endCap=true}={}){
