...
export default function generate(THREE) {
  const family = 'grille', kind = 'reinforced', root = new THREE.Group();
  ...
  if(kind==='start'||kind==='end') root.traverse(n=>{
    if(!n.isMesh)return;
    ...
    for(let i=0;i<p.count;i++) {
      const t=THREE.MathUtils.clamp((kind==='start'?p.getX(i)+12:12-p.getX(i))/8,0,1);
