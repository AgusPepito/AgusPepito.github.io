...
import { trackAt, roadPoint } from '../src/track.js';
  ...
  for (const aspect of [390 / 844, 907 / 958, 16 / 9, 844 / 390]) {
    ...
    for (const cameraShift of [true, false]) for (const raceBlend of [0, 0.25, 0.5, 0.75, 1]) for (let distance = 30; distance < 2100; distance += 10) {
      for (const offset of [-7, 0, 10]) for (const side of [-1, 0, 1]) {
        ...
        const lateral = side * (road.width / 2 - 1);
        const sim = { distance, offset, raceBlend, lateral, s: distance + offset, ...roadPoint(distance + offset, lateral) };
        ...
        const projected = new Vector3(sim.x, 0.975, sim.z).project(camera);
